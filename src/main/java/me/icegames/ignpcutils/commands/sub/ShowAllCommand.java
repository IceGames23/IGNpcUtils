...
public class ShowAllCommand implements SubCommand {
    ...
    public ShowAllCommand(IGNpcUtils plugin, me.icegames.ignpcutils.util.NPCResolver resolver) {
        this.plugin = plugin;
            ...
            Bukkit.getScheduler().runTask(plugin, () -> {
                ...
                if (npcIds.size() == 1) {
                    SubCommand.sendMessage(sender,
                            MessageUtil.getMessage(plugin.getMessagesConfig(), "npc_removed", "%id%",
                                    String.valueOf(npcIds.get(0))),
                            silent);
                } else {
                    SubCommand.sendMessage(
                            sender, MessageUtil.getMessage(plugin.getMessagesConfig(), "npc_removed_multi", "%count%",
